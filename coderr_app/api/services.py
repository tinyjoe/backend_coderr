...
def validate_details(self, value):
        ...
        if not isinstance(value, list):
            raise serializers.ValidationError({'message': 'Details need to be a list of OfferDetail objects.'})
        if len(value) != 3:
            raise serializers.ValidationError({'message': 'An offer must contain exactly 3 details.'})
        return value
...
def update_offer_detail(self, details_data, existing_details, serializer):
        ...
        for detail_data in details_data:
            ...
            if not offer_type:
                 raise serializers.ValidationError({'message': 'Um das Angebotsdetail zu aktualisieren, ist der Angebotstyp erforderlich.'})
